...
import { ClipboardList, Search, Filter, Calendar, User, Projector, CheckCircle2, XCircle } from "lucide-react";
...
const AdminReservations = () => {
  ...
  const fetchReservations = async () => {
    try {
      ...
      const response = await api.get(`/reservations/all?search=${search}${statusParam}`);
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Monitor and manage all DataShow bookings across the institution.
        </p>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border p-5 shadow-sm space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by professor, room, or datashow..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full rounded-3xl border border-border bg-background px-10 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          </div>
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <select
                ...
                className="w-full appearance-none rounded-3xl border border-border bg-background px-10 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
              >
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {loading ? (
              <tr><td colSpan="5" className="px-6 py-10 text-center text-muted-foreground">Loading reservations...</td></tr>
            ) : reservations.length === 0 ? (
              <tr><td colSpan="5" className="px-6 py-10 text-center text-muted-foreground">No reservations found matching your criteria.</td></tr>
            ) : (
